...
We model the bounding box refifinement as a residual
 learning problem. For an initially regressed bounding box
 (l’, t’, r’, b’), we fifirst extract the star-shaped representation
 to encode it. Then, we learn four distance scaling factors
 (∆l, ∆t, ∆r, ∆b) to scale the distance vector so that the
 refifined bounding box represented by (l, t, r, b) = (∆l×l’,
 ∆t×t’, ∆r×r’, ∆b×b’) is closer to the ground truth.
